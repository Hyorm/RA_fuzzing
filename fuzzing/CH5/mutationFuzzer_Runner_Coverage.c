...
int main(int argc, char** argv){
	...
	printf("ps choose %s\n", ps.choose(ps, mf.population).seed);
	...
	Population popop = {se, 3};

	for(int i = 0; i < 100000 ; i++){
		
		if(strcmp("a", ps.choose(ps, popop).seed) == 0)
			hits[0]++;
		else if(strcmp("b", ps.choose(ps, popop).seed) == 0)
			hits[1]++;
		else
			hits[2]++;
	}
...
float normalizedEnergy_PowerSchedule(PowerSchedule ps, Population pop){
	...
	return (float)sum / (float)pop.pop_size;
}
...
SeedEnergy choose_PowerSchedule(PowerSchedule ps, Population pop){
	...
	int p = rand()%100+1;
	float a = 100.0/(float)pop.pop_size * norm_energy;
...
MutationFuzzer reset_MutationFuzzer(MutationFuzzer mf){
	/*
	if(mf.population.pop_size != NULL && mf.population.pop_size != 0){
		for(int i = 0; i< mf.population.pop_size ; i++)
	*/	//	free(mf.population);//.se[i]);
	
	Population popmf;
	popmf.se = malloc(sizeof(SeedEnergy)*sizeof(mf.seed)/sizeof(char*));
	
	for(int i = 0; i< sizeof(mf.seed)/sizeof(char*);i++){
		popmf.se[i].seed = mf.seed[i];
	}

	popmf.pop_size = sizeof(mf.seed)/sizeof(char*);
	mf.seed_index = 0;
...
MutationFuzzer create_candidate_MutationFuzzer(MutationFuzzer mf){
	...
	char** buffer_pop;
	int buf_idx = 0;
	buffer_pop = (char**)malloc(sizeof(char*)*(mf.population.pop_size+trials));
	for(int i = 0; i < mf.population.pop_size ; i++){
		buffer_pop[i] = (char*)malloc(sizeof(char)*strlen( mf.population.se[i].seed));
		strcpy(buffer_pop[i], mf.population.se[i].seed);
		buf_idx++;
	...
	mf.population.se = (char**)malloc(sizeof(SeedEnergy)*buf_idx);
	mf.population.pop_size = 0;
	for(int i = 0; i < buf_idx ; i++){
		mf.population.se[i].seed = buffer_pop[i];
		mf.population.pop_size++;
